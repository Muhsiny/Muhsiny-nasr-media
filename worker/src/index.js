 ...
 if(processMatch&&request.method==='POST'){
  ...
  try{
   ...
   for(const item of outputs){
    ...
    await env.DB.prepare("UPDATE pipeline_runs SET status='completed',output_json=?,confidence_score=?,completed_at=datetime('now') WHERE article_id=? AND agent_key=?").bind(JSON.stringify(item.output),confidence,article.id,item.key).run();
   }
